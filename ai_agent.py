...
def analyze_care_context(context, pet, owner):
    ...
    Returns a dict with either 'suggestions' (list) or 'error' (str).
    """
    ...
    prompt = f"""You are an expert pet care assistant.
Pet: {pet.name} ({pet.species})
Owner's Daily Budget: {owner.available_minutes} minutes
User's Context: "{context}"
Current Pending Tasks:
{chr(10).join(current_tasks) if current_tasks else "  (none)"}
...
- target_title: (for modify/remove) the exact title of the existing task
- task_data: (for add/modify) a dictionary with any of these fields:
